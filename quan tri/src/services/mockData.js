export const generateMockDocuments = () => {
  ...
  return Array.from({ length: 20 }).map((_, index) => {
    ...
    const aiConfidence = Math.floor(Math.random() * 40) + 60; // 60 to 99
    ...
    return {
      ...
      status: status === 'Đã xử lý' && isLowConfidence ? 'Cần kiểm tra' : status, // force need review if low conf
      ocrData: {
...
export const addMockDocument = (file) => {
